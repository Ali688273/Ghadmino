...
data class PersonalRecords(val bestDaySteps:Int,val bestDayDate:String,val best7DayTotal:Int,val best30DayTotal:Int,val longestStreak:Int,val currentStreak:Int,val activeDays30:Int,val bestWorkoutDistanceMeters:Double,val bestWorkoutMinutes:Int)
...
object PersonalRecordsRepository {
    fun calculate(context: Context): PersonalRecords {
        ...
        return PersonalRecords(best?.second ?: 0,best?.first ?: "-",rolling(rows,7),rolling(rows,30),longest(rows),current(rows),rows.take(30).count { row -> row.second>0 },workouts.maxOfOrNull { it.distanceMeters } ?: 0.0,workouts.maxOfOrNull { it.durationMinutes } ?: 0)
    }
