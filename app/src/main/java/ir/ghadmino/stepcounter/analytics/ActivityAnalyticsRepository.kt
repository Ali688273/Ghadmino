...
object ActivityAnalyticsRepository {
 ...
 fun report(c:Context):ActivityReport{
  ...
  return ActivityReport(t,goal,(t*100L/goal).coerceIn(0,100).toInt(),distance,calories,w,pw,if(pw==0){if(w>0)100 else 0}else((w-pw)*100f/pw).roundToInt(),d30.sumOf{it.second},d30.sumOf{it.second}/30,if(d90.isEmpty())0 else d90.sumOf{it.second}/90,best,d30.count{it.second>0},longest(d30),if(remain==0||rate==0)0 else ceil(remain*60.0/rate).toInt(),rate)
 }
